...
public class LecturaEnemigos {
    ...
    public static ArrayList<Personaje> leerEnemigos() {
        ...
        try (BufferedReader br = new BufferedReader(
                new InputStreamReader(new FileInputStream(new File(App.class.getResource("data/enemigos.csv").toURI())),
                        StandardCharsets.UTF_8))) {
            String linea = br.readLine();
            while ((linea = br.readLine()) != null) {
                String[] atributos = linea.split(",");
                try {
                    enemigos.add(new Enemigo(Integer.parseInt(atributos[0]), Integer.parseInt(atributos[1]),
                            Integer.parseInt(atributos[2]), Integer.parseInt(atributos[3]), atributos[4]));
